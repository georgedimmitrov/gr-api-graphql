...
function translate(lang, str) {
  const apiKey = 'gottahavebillingaccount';
    ...
    books: {
      ...
      resolve: xml => {
        ...
        return Promise.all(ids.map(id =>
          fetch(`https://www.goodreads.com/book/show/${id}.xml?key=${config.goodreadsApikey}`)
    ...
    fields: () => ({
      author: {
        ...
        resolve: (root, args) => fetch(
            `https://www.goodreads.com/author/show.xml?id=${args.id}&key=${config.goodreadsApikey}`
          )
          .then(response => response.text())
          .then(parseXML)
      }
